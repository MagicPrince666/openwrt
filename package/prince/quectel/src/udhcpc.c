...
#define XDLM_QUECTEL_CM_PORT 8558
...
void udhcpc_start(PROFILE_T *profile) {
    ...
    int xdlm_fd;
    unsigned char dns_tmp[16];
    int ret;
	int local_idx;

	local_idx = atoi(&ifname[strlen(ifname) - 1]);
...
//use the same policy for all modules
    ...
    if (profile->rawIP && ((profile->IPType==0x04 && profile->ipv4.Address)))
    {
		...
		//xag route setting
		snprintf(shell_cmd, sizeof(shell_cmd), "ip route add default via %d.%d.%d.%d dev %s table %d",
            gw[3], gw[2], gw[1], gw[0], ifname, local_idx + 2);
		dbg_time("%s", shell_cmd);
        system(shell_cmd);
        ...
        if (dns1[0])
        {
            ...
            snprintf(shell_cmd, sizeof(shell_cmd), "echo -n \"nameserver %d.%d.%d.%d\nnameserver %d.%d.%d.%d\n\" > /etc/%s.conf",
                dns1[3], dns1[2], dns1[1], dns1[0], dns2[3], dns2[2], dns2[1], dns2[0], ifname);
            system(shell_cmd);
            snprintf(shell_cmd, sizeof(shell_cmd), "cat /etc/%s.conf >> /tmp/resolv.conf.auto", ifname);
            system(shell_cmd);

            memset(&qmsg, 0, sizeof(struct cm_msg));
            strncpy(&qmsg.dev_name, ifname, strlen(ifname));
            snprintf(dns_tmp, dns_tmp, "%d.%d.%d.%d", dns1[3], dns1[2], dns1[1], dns1[0]);
            strncpy(&qmsg.dns_addr, dns_tmp, strlen(dns_tmp));
            
            memset(&server_addr, 0, sizeof(struct sockaddr_in));
            server_addr.sin_family = AF_INET;
            server_addr.sin_port = htons(XDLM_QUECTEL_CM_PORT);
            server_addr.sin_addr.s_addr = inet_addr("127.0.0.1");
            ret = sendto(xdlm_fd, (char *)&qmsg, sizeof(qmsg), 0, (struct sockaddr *)&server_addr, sockaddr_len);
            if (ret < 0)
                dbg_time("errno: %d(%s)", errno, strerror(errno));
            dbg_time("%s %d", __func__, __LINE__);
        }   
